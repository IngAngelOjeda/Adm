...
export class SistemasListComponent implements OnInit {
  ...
  constructor(
    ...
    this.breadcrumbService.setItems([
      { label: "Datos de Oee" },
      { label: "Sistemas", routerLink: ["/oee/sistemas"] },
    ]);
  ...
  loadData() {
    ...
    this.service.getAll(this.filter, this.pageSize, this.start, this.sortField, this.sortAsc, this.searchFormGroup.value, this.id, this.permiso)
      .subscribe((response) => {
        this.loading = false;
        if (response) {
          this.sistemas = response.data?.lista;
          this.totalRecords = this.checkPermission('sistemas:listar') ? response.data?.totalRecords : 0;
        }
      }, error => {
        this.loading = false;
        console.log(error);
  ...
  updateStatus(row: Sistemas) {
    this.confirmationService.confirm({
      message: `Está seguro que desea ${row.estado === 'I' ? 'habilitar' : 'inactivar'} <b>${row.nombre}</b>?`,
      header: '¡Atención!',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No',
      accept: () => {
        this.service.updateStatus(row.idSistema).subscribe(response => {
  ...
  delete(row: Sistemas) {
    this.confirmationService.confirm({
      message: `Está seguro que desea eliminar <b>${row.nombre}</b>?`,
      header: '¡Atención!',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No',
      accept: () => {
        this.service.delete(row.idSistema).subscribe(response => {
          if (response?.code == 200) {
            this.messageService.add({ severity: 'success', summary: 'Operación exitosa', detail: response.message, life: 3000 });
            this.loadData();
          } else {
      ...
      if (response) {
        ...
        switch (band) {
          ...
          case 'csv':
            ...
            setTimeout(() => {
              this.exportCsv('Sistemas_export_' + new Date().getTime() + '.csv', body);
  ...
  exportExcel(dataExport) {
    ...
    this.saveAsExcelFile(excelBuffer, "Sistemas");
  }
  ...
  exportJson(listar) {
    ...
    FileSaver.saveAs(blob, "Sistemas" + '_export_' + new Date().getTime() + EXTENSION);
  }
  ...
  getColumnsExportExcel(data) {
    ...
    for (let row of data) {
      body.push({
        'Nombre': row.nombre,
        'Propósito del Sistema': row.objetoProposito,
        'Área Responsable': row.areaResponsable,
  ...
  generarPDF() {
    ...
    this.service.getInforme(this.searchFormGroup.value, "pdf", this.id, this.permiso).subscribe(
      (data: Blob) => {
        FileSaver.saveAs(data, 'Reporte Sistemas.pdf');
      },
      error => {
