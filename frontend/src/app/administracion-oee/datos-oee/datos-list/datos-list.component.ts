...
export class DatosListComponent implements OnInit {
  ...
  public datos: DatosOee[];
  public entity: DatosOee;
  public cols: 3;
        ...
        if(response) {
          ...
          switch ( band ) {
            ...
            case 'csv':
              ...
              setTimeout(() => {
                  this.exportCsv('Datos_Oee_export_' + new Date().getTime() +'.csv', body);
...
exportExcel(dataExport) {
    ...
    this.saveAsExcelFile(excelBuffer, "Datos-Oee");
}
...
exportJson(listar) {
  ...
  FileSaver.saveAs(blob, "Datos_Oee" + '_export_' + new Date().getTime() + EXTENSION);
}
...
getColumnsExportExcel(data) {
    ...
    for (let row of data) {            
        body.push({
            'Información': row.descripcionOeeInformacion,
            'Tipo de Dato': row.tipoDato.descripcionTipoDato,
            'Descripción OEE': row.oee.descripcionOee,
            'Estado': row.estadoOeeInformacion == "A" ? 'ACTIVO' : 'INACTIVO',
...
/* generarPDF() {
  this.service.getInforme("pdf", this.id).subscribe(
    (data: Blob) => {
      FileSaver.saveAs(data, 'Reporte Datos OEE.pdf');
    },
    error => {
...
generarPDF() {
  ...
  openLinkPortalPY() {
    // Redirigir al Portal Paraguay de acuerdo a la OEE del usuario
    this.oeeNombreCorto = this.storageManager.getCurrenSession().usuario.oee.descripcionCorta;
    if(this.oeeNombreCorto != null)
    {
      window.open('https://www.paraguay.gov.py/oee/'+this.oeeNombreCorto, '_blank');
    }
  }
