...
import { UserSession } from 'src/app/shared/models/usersession.model';
import { StorageManagerService } from 'src/app/shared/services/storage-manager.service';
import { Tipos } from 'src/app/shared/models/tipos.models';
...
export class InstitucionListComponent implements OnInit {
    ...
    public instituciones: Oee[];
    public entity: Oee;

    // Get Current Institución
    ...
    constructor(
      ...
      this.currentUser = this.storageManager.getCurrenSession();
      this.institucion = this.currentUser.usuario?.oee;

      this.breadcrumbService.setItems([
        { label: "Administración" },
        { label: "Institución", routerLink: ["/administracion/institucion"] },
      ]);
    ...
    loadData() {      
      ...
      this.service.getAll(this.filter, this.pageSize, this.start, this.sortField, this.sortAsc, this.searchFormGroup.value).subscribe((response) => {
        this.loading = false;
        if(response) {
            this.instituciones = response.data?.lista;
    ...
    exportData(band) {
        ...
        this.service.getAll(this.filter, -1, this.start, this.sortField, this.sortAsc, this.searchFormGroup.value).subscribe((response) => {
            if(response) {
              ...
              switch ( band ) {
                ...
                case 'csv':
                  ...
                  setTimeout(() => {
                      this.exportCsv('Instituciones_export_' + new Date().getTime() +'.csv', body);
    ...
    exportExcel(dataExport) {
        ...
        this.saveAsExcelFile(excelBuffer, "Instituciones");
    }
    ...
    exportJson(listar) {
      ...
      FileSaver.saveAs(blob, "Instituciones" + '_export_' + new Date().getTime() + EXTENSION);
    }
    ...
    getColumnsExportExcel(data) {
        ...
        for (let row of data) {            
            body.push({
                'Descripcion': row.descripcionOee,
                'Descripcion Corta': row.descripcionCorta,
                /* 'Entidad': row.entidad["descripcionEntidad"],  */
                'Estado': row.estadoOee == "A"? 'ACTIVO' : 'INACTIVO',
            });
        }
        //console.table(body);
        return body;
    ...
    delete(row: Oee) {
      this.confirmationService.confirm({
        message: `Está seguro que desea inactivar la institución <b>${row.descripcionOee}</b>?`,
        header: '¡Atención!',
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
          this.service.delete(row.id).subscribe(response => {
            if(response && response?.code == 200) {
              this.messageService.add({severity: 'success', summary: 'Operación exitosa', detail: `La institución ${row.descripcionOee} se inactivó con éxito`, life: 3000});
              this.loadData();
            } else {
              this.messageService.add({severity: 'error', summary: 'Atención!', detail: `No se pudo inactivar.`, life: 3000});
            }
    ...
    generarPDF() {
      this.service.getInforme(this.searchFormGroup.value, "pdf", this.id).subscribe(
        (data: Blob) => {
          FileSaver.saveAs(data, 'Reporte Instituciones.pdf');
        },
        error => {
