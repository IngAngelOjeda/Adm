...
@Component({
  selector: 'app-planes-tic-list',
  templateUrl: './planes-tic-list.component.html',
  styleUrls: ['./planes-tic-list.component.scss'],
  providers: [DecimalPipe],
})
export class PlanesTicListComponent implements OnInit {
  ...
  public planes: PlanesTic[];
  /* public oee: Oee[]; */
  public entity: PlanesTic;
  public cols: 3;
  ...
  constructor(
    ...
    this.breadcrumbService.setItems([
      ...
      { label: "Planes TIC", routerLink: ["/oee/planes-tic"] },
    ]);
      ...
      .subscribe((response) => {
        ...
        if(response) {
          ...
          /* array.forEach(e => {
            ...
            this.planes = response.data?.lista;
        ...
        if(response) {
          ...
          switch ( band ) {
            ...
            case 'csv':
              ...
              setTimeout(() => {
                  this.exportCsv('Panes_Tic_export_' + new Date().getTime() +'.csv', body);
...
exportExcel(dataExport) {
    ...
    this.saveAsExcelFile(excelBuffer, "Planes-Tic");
}
...
exportJson(listar) {
  ...
  FileSaver.saveAs(blob, "Planes_Tic" + '_export_' + new Date().getTime() + EXTENSION);
}
...
getColumnsExportExcel(data) {
    ...
    for (let row of data) {            
        body.push({
            'Año': row.anho,
            'fecha': row.fecha,
            'link Plan': row.linkPlan,
            'Cant. Func. TIC': row.cantidadFuncionariosTic,
            'Cant. Func. Administración': row.cantidadFuncionariosAdmin,
            'Presupuesto Anual TIC': row.presupuestoTicAnual,
...
// FUNCION EXPORTAR EN FORMATOS
  ...
  delete(row: PlanesTic) {
    this.confirmationService.confirm({
      message: `Está seguro que desea dar de baja este Plan TIC?`,
      header: 'Eliminar',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No',
      accept: () => {
        this.service.delete(row.idPlan).subscribe(response => {
          console.log('status',response);
          if(response?.code == 200) {
          /* if(response?.status == 200) { */
            this.messageService.add({severity: 'success', summary: 'Operación exitosa', detail: `El Plan TIC se dió de baja con éxito`, life: 3000});
            this.loadData();
          } else {
  ...
  updateStatus(row: PlanesTic) {
    this.confirmationService.confirm({
      message: `Está seguro que desea ${ row.estado ? 'inactivar' : 'habilitar' } este Plan?`,
      header: '¡Atención',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No',
      accept: () => {
        this.service.updateStatus(row.idPlan).subscribe(response => {
            if(response?.code == 200) {
  ...
  generarPDF() {
    ...
    this.service.getInforme(this.searchFormGroup.value, "pdf", this.id, this.permiso).subscribe(
      (data: Blob) => {
        FileSaver.saveAs(data, 'Reporte Planes TIC.pdf');
      },
      error => {
