...
export class UsuarioListComponent implements OnInit {
  ...
  constructor(
    ...
    this.breadcrumbService.setItems([
      ...
      { label: "Usuarios", routerLink: ["/administracion/usuario"] },
    ]);
  ...
  loadRoles() {
    this.rolService.getRoles().subscribe((response) => {
        if(response) this.roles = this.getSelectRolData(response.data);
    });
  }
  ...
  delete(row: Usuario) {
    this.confirmationService.confirm({
      message: `Está seguro que desea dar de baja el usuario <b>${row.nombre} ${row.apellido}</b>?`,
      header: 'Eliminar',
  ...
  updateUserStatus(row: Usuario) {
    const usuario = new Usuario();
    usuario.estado = row.estado;

    const status = row.estado ? 'activar': 'inactivar';

    this.confirmationService.confirm({
      message: `Está seguro que desea ${status} el usuario <b>${row.username}</b>?`,
      header: status.charAt(0).toUpperCase() + status.slice(1),
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No', 
      accept: () => {
        this.service.updateStatus(row.idUsuario, usuario).subscribe(response => {
            if(response?.code == 200) {
                this.messageService.add({severity: 'success', summary: 'Operación exitosa', detail: response.message, life: 3000});
                this.loadData();
            } else {
      ...
      reject: () => {
        if (row.estado) row.estado = false; else row.estado = true;
      }
    });
  }
      ...
      if (response) {
        ...
        switch (band) {
          ...
          case 'csv':
            ...
            setTimeout(() => {
              this.exportCsv('Usuarios_export_' + new Date().getTime() + '.csv', body);
  ...
  exportJson(listar) {
    ...
    FileSaver.saveAs(blob, "Usuarios" + '_export_' + new Date().getTime() + EXTENSION);
  }
  ...
  getColumnsExportExcel(data) {
    ...
    for (let row of data) {
      body.push({
        ...
        'Nombre Usuario': row?.username,
        'Rol': row?.roles != undefined ? row.roles.map(role => role.descripcion).join(', ') : '',
        'Institución': Array.isArray(row?.oee) ? row.oee.map(oee => oee.descripcionOee).join(', ') : (row.oee ? row.oee.descripcionOee : ''),
        'Fecha de Expiración': row?.fechaExpiracion,
        'Estado': row?.estado && row?.estado != undefined ? 'ACTIVO' : 'INACTIVO' 
