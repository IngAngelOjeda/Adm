...
@Component({
  ...
  styleUrls: ['./dominio-list.component.scss']
})
export class DominioListComponent implements OnInit {
  ...
  constructor(
    ...
    this.breadcrumbService.setItems([
      { label: "Datos de Oee" },
      { label: "Dominios", routerLink: ["/oee/dominio"] },
    ]);
  ...
  loadData() {
    ...
    this.service.getAll(this.filter, this.pageSize, this.start, this.sortField, this.sortAsc, this.searchFormGroup.value, this.id, this.permiso).subscribe((response) => {
      this.loading = false;
      if (response) {
        this.dominios = response.data?.lista;
        this.oee = response.data?.lista;
        this.totalRecords = this.checkPermission('dominio:listar') ? response.data?.totalRecords : 0;
      }
    }, error => {
      this.loading = false;
      console.log(error);
  ...
  openSubdominios(row: Dominio) {
    this.showSubdominiosList = true;
    this.entity = { ...row };
  }
  openEdit(row: Dominio) {
    this.showDialog = true;
    this.entity = { ...row };
  }
  ...
  changeDialogSubdominioVisibility($event) {
    this.showSubdominiosList = $event;
  }

  updateStatus(row: Dominio) {
    this.confirmationService.confirm({
      message: `Está seguro que desea ${row.estado ? 'inactivar' : 'habilitar'} el dominio <b>${row.dominio}</b>?`,
      header: '¡Atención!',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No',
      accept: () => {
        this.service.updateStatus(row.idDominio).subscribe(response => {
  ...
  delete(row: Dominio) {
    this.confirmationService.confirm({
      message: `Está seguro que desea eliminar el dominio <b>${row.dominio}</b>?`,
      header: '¡Atención!',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'No',
      accept: () => {
        this.service.delete(row.idDominio).subscribe(response => {
          if (response?.code == 200) {
            this.messageService.add({ severity: 'success', summary: 'Operación exitosa', detail: response.message, life: 3000 });
            this.loadData();
          } else {
      ...
      if (response) {
        ...
        switch (band) {
          ...
          case 'csv':
            ...
            setTimeout(() => {
              this.exportCsv('Dominios_export_' + new Date().getTime() + '.csv', body);
  ...
  exportExcel(dataExport) {
    ...
    this.saveAsExcelFile(excelBuffer, "Dominios");
  }
  ...
  exportJson(listar) {
    ...
    FileSaver.saveAs(blob, "Dominios" + '_export_' + new Date().getTime() + EXTENSION);
  }
  ...
  generarPDF() {
    ...
    this.service.getInforme(this.searchFormGroup.value, "pdf", this.id, this.permiso).subscribe(
      (data: Blob) => {
        FileSaver.saveAs(data, 'Reporte Dominios.pdf');
      },
      error => {
